...
| Frontend | Planner can complete the workflow from login through report. | Vue app (`frontend/`); the Playwright test `tests/e2e/workflow.spec.js` covers login → upload → run → review → map → PDF report, plus read-only access for a second planner and the admin paths (core curriculum, NLP defaults, users, audit); runs in CI | ✅ |

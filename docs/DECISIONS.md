...
- **The core reference** is every parsed NUC Core Reference document in the library, not just those in the session. The run endpoint refuses to start without one (`409 NO_CORE_REFERENCE`), so compute isn't wasted on a run that cannot finish. Core segments are text blocks of 3–60 words (longer blocks become sentence windows, and unpunctuated runs are cut at 60 words), which keeps course titles such as "CSC 421: Cloud Computing" as their own segments. Course-structure tables, which the PDF parser returns as one run-on block, are split into one segment per course ("CSC 301 Operating Systems", with the units and hours columns removed); a block counts as a table only when most of its rows start with a course code and end in numeric columns. Without this, a whole table became a single vague segment that matched almost anything. Checked on a 109-page synthetic document laid out like the CCMAS: parsed in 2 s, 1,476 segments.
